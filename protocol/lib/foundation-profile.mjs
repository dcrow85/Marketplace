...
export const FOUNDATION_OPERATIONS = deepFreeze([
  ...
  ["intent.put", true, ["consume_grant_disclosure_budget"], "private_state_write", "active-intent.schema.json", "storedObjectResponse", true, "principal_signature_and_data_grant", "intent_storage", "write_object", "records_principal_signed_intent_only"],
  ["intent.get", false, ["consume_grant_disclosure_budget"], "private_read", "objectRefRequest", "active-intent.schema.json", true, "data_grant", "intent_read", "read_local", "none"],
  ["data_grant.get", false, ["consume_grant_disclosure_budget"], "private_read", "objectRefRequest", "data-grant.schema.json", true, "data_grant", "grant_read", "read_local", "none"],
...
export function operationTuple(operation) {
  return [
    ...
    operation.access_state_effects,
...
export const EXACT_FOUNDATION_OPERATION_TUPLES = deepFreeze(
  FOUNDATION_OPERATIONS.map(([name, objectStoreMutating, accessStateEffects, consequence, request, response, grant, authorization, purpose, use, authority]) =>
    Object.freeze([
      name,
      objectStoreMutating,
      accessStateEffects,
