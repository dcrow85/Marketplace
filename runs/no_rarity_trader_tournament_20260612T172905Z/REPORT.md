# No Rarity Trader Tournament: 2026-06-12T17:29:05.898118+00:00

## Result

- Pass: `True`
- Attempts: `18/18`
- Mutation proofs passed: `True`
...
- `PASS` Off-set Umbreon want must not bind to a row
  - Expected: `no_in_set_match`
  - Observed: `no_in_set_match`
  - Inferred stance: `want`
  - Next: tell the human which terms had no in-set match, confirm whether they meant a different set or era, do not attach this intent to a different catalog row without explicit confirmation
...
- `PASS` Mewtwo with declared slab-level evidence
  ...
  - Observed: `human_or_verifier_review`
  - Decision: `human_or_verifier_review`
  - Human summary: A high-value slab/cert claim from thin trust needs review before funding.
  - Missing: slab front image, slab back image, label close-up, cert lookup screenshot or signed lookup receipt, card front/back visible through slab
- `PASS` Off-set slabbed Espeon claim must not bind to a row
  - Expected: `no_in_set_match`
  - Observed: `no_in_set_match`
  - Inferred stance: `want`
  - Next: tell the human which terms had no in-set match, confirm whether they meant a different set or era, do not attach this intent to a different catalog row without explicit confirmation
...
- `PASS` Seller attention plan
  ...
  - Evidence asks: full front image, full back image, sharp lower-right rarity-symbol crop with surrounding border, fresh nonce possession image, source-family or provenance note for unresolved missing-symbol overlap, explicit public-sharing permission for any photos and condition notes
...
### weaken_request_evidence_to_low_friction

- Target: classify_outcome: treat request_evidence decisions as low_friction_pass
- Expected detection: at least one evidence-wall case fails its expected label
- Detected: `True`
- Failing cases:
  - `holo_grail_buyer` / Charizard with low evidence from thin seller: expected `request_evidence`, observed `low_friction_pass`
  - `adversarial_seller` / Catalog-match-as-possession attack on Charizard: expected `request_evidence`, observed `low_friction_pass`
