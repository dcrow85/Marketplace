# Fingerprint Collision Drill: fingerprint_collision_drill_20260519T165803Z

- Generated: `2026-05-19T16:58:06.258060+00:00`
...
The exact same active fingerprint hash is blocked by the EVM. Different hashes that look like the same physical card, a stale photo, or a front/back mismatch are detected off-chain and turned into buyer-signed `FingerprintChallenge` packets. Once the challenge is anchored, route commitment reverts until the buyer clears or waives it.
...
| Same PSA cert reused with different photos | `3, 4` | `same_cert_active_elsewhere:psa-cert-hash-65081234:trade_3` | `different_hash_allowed_until_detector_opens_challenge` |
| Prior marketplace image reused without custody nonce | `5` | `prior_market_ref_without_current_custody_nonce`<br>`stale_capture_window` | `buyer_challenge_blocks_route` |
| Front and back markers appear to be different cards | `6` | `front_back_match_group_mismatch` | `buyer_challenge_blocks_route` |
...
- Transactions: `11`
- Outcome: `different_hash_allowed_until_detector_opens_challenge`
- Observations:
...
- Open challenges currently pause route commitment but do not define a full seller cure flow beyond buyer clearing or waiving the challenge.
...
Add a scoped `FingerprintAttestation` or extend `VerifierScopeAttestation` profiles for image/cert collision review, then run the same drill with verifier-signed challenge satisfaction packets: fresh nonce photo accepted, stale cert rejected, mixed front/back escalated.
