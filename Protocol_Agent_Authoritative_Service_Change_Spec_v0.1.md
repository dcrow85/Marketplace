...
**Status:** the seventeenth frozen candidate,
`b8a84410f8770f4b4c045f2868af8852f2ebf5d4`, received one independent
pass and two independent rejections. This working tree contains eighteenth-pass
remediation under review. It is not independently re-audited, implemented as a
durable SQLite service, or conforming.
...
idempotency map with its exact durable snapshot. It also compares the candidate
receiver record with the already verified account-to-namespace and
authentication-handle bindings before deriving or looking up a database key.
Only after that complete preflight may it decide that the requested
receiver-private database key is absent or validate the requested pair. A
mismatch in history or any idempotency row, including an unrelated existing row,
returns `idempotency_integrity_invalid`; an incompatible incoming receiver
binding returns `receiver_authentication_invalid`. Both carry `kernel:null`, an
empty callback trace, unchanged kernel/sidecar snapshots, and no callback
invocation.
...
Immediately before publication, the wrapper runs the complete historical
verifier against the staged sidecar—including the new receiver record, exact
request signature, query commitment, operational-version inventory, and signed
observation. A staged inconsistency is a persistence-stage wrapper failure and
publishes nothing.
...
re-signed envelope cannot be relinked to a different rich row. Historical
verification recalculates the stored envelope's operation fingerprint from its
exact semantic fields and requires the origin rich row to carry that exact
fingerprint. The wrapper does not compare that proven historical fingerprint
with a new request during rich-row preflight; after history is proven coherent,
the unchanged frozen callback owns the new request-conflict decision.
...
The Phase-A adapter builds the complete manifest, projections, and sequence-zero
versions in a detached staged sidecar, verifies that staged value, and publishes
it only after acceptance. A failed seal leaves both the live sidecar and
bootstrap-open state byte-identical. Repeating the exact seal recomputes the
candidate, verifies the already sealed history, and returns the existing
manifest hash; it does not create another genesis row or close an invalid store.
The Phase-A fixture pins the exact manifest hash
`sha-256:4d611336b590521b5ec06a659d958912ae4b6fe2e5c2f7b30a28a386abc3f8aa`;
the verifier accepts neither a self-consistently rebound substitute nor an
additional sequence-zero row.
...
verifier requires every version sequence to be a safe integer from zero through
the latest committed global sequence, cross-binds every version wrapper column
to its exact closed projection, and requires the complete set of sequence-zero
version tuples to equal the pinned genesis manifest rows exactly. A non-genesis
version must correspond to a write dependency in that exact commit.
`operational_versions` storage order is intentionally irrelevant; the sorted
tuple inventory and reconstructed history must be exact. An omitted, duplicate,
extra, backdated, negative-sequence, unwritten-positive, or uncommitted-future
version fails. Per-operation dependency rows/commits, service commits, scope
commits, observations, repository rows, validation/receiver/host/request rows,
access traces, and callback witnesses are ordered by their exact operation
sequence and fail on omission, duplication, addition, or reordering.
Dependency rows may name only committed operation sequences one through the
current global sequence; genesis, negative, and future dependency rows fail
before per-commit reconstruction. Canonical signed-object witnesses
are schema-validated, have their self-hash and signature-hash bindings
recalculated, and have every signature verified against the transaction-visible
historical key before independently re-deriving each object ref and identity
...
The genesis row uses `global_commit_sequence:0`,
`previous_global_sequence:null`, `transaction_kind:genesis`,
`committed_at:<sealed genesis time>`, and `observation_ref_key:null`.
Every later row uses its exact consecutive global commit sequence, prior
sequence, operation/replay kind, committed observation ref, and authenticated
commit time. The closed row neither aliases `global_commit_sequence` as
`global_sequence` nor omits `committed_at`.
...
Sets, and validation-key resolver with ordered presence, exact canonical
before/after value witnesses, and hashes recomputed from those witnesses. The
verifier cross-binds each witness to the authoritative runtime binding,
validation key, nonce, idempotency row, object, identity, URI, ACL, DataGrant,
or grant state visible at that sequence. Signed-object witnesses are also
schema-checked, self-hash/signature-hash checked, and cryptographically verified
with the exact transaction-visible historical validation key. That one trace is
the sole source for the transaction's dependency rows and aliases, including
each exact access kind. A coherently re-signed value/hash or access-kind
substitution, unsupported access, unconsumed access, missing required
store/index surface, or noncontiguous trace fails the transaction rather than
allowing the wrapper to hand-author an incomplete dependency set.
...
All per-operation wrapper collections are positional ledgers: row index and
embedded global sequence must agree for every committed operation. Dependency
rows are sorted within each operation by their closed entry key. This positional
rule does not apply to `operational_versions`; those are verified as an exact
order-independent tuple set so database storage order cannot change meaning.
...
resolves the envelope signing key from the transaction-visible validation-key
version, checks controller/lifecycle/signed-hash metadata, and verifies the
Ed25519 signature. It also recomputes `query_commitment` from the exact envelope,
receiver-derived authentication, registered operation contract, and host-context
...
changed-fingerprint conflict, a second same-operation origin under a distinct
idempotency key and its exact-key replay, five separately injected
result-object/ACL replay faults, four post-callback
response-schema/observation/persistence/commit fault boundaries, three
unexpected non-fixture exception paths at the observation, persistence, and
commit boundaries, and one grant-consumption failure injected after new
idempotency staging. The successful replay commits a second nonce, owner/global
sequence, scope root, replay observation, and repository row while retaining
the original result and charge. The two-key scenario keeps two rich and frozen
rows with the same operation fingerprint but different signed structural-key
commitments; replay of the second key must point to the second origin
observation, never whichever row appears first. Each replay corruption captures
the unchanged frozen
...
Unexpected thrown errors are classified at their transaction-owned boundary
and never collapse into the frozen service's generic
...
its CLI summary reports 41 exercised cases, and fresh process executions must
...
Negative controls add duplicate and extra rows,
swap every positional per-operation collection and same-operation dependency
row order,
...
make a receiver namespace operation-qualified, alter only the persisted request
signature proof, substitute a re-signed query commitment, add an uncommitted
future operational version, add a backdated sequence-zero version, rebind a
genesis manifest with an extra row, add a positive version without a matching
write, alter service-commit sequence/time, and invalidate the later suffix of a
historical profile chain.
...
expired, or mismatched contexts. Context installation consumes a one-shot,
store-issued lease token on its first attempt. Each token is bound to the exact
global store sequence at issuance and expires after any intervening committed
transaction advances that sequence. A nested installation cannot
replace the active context; the active request can still complete, and neither
its token nor the rejected nested token can be reused. After sealed bootstrap,
a direct raw-store call without a context returns
`receiver_authentication_required` before a callback, trace, or state delta;
there is no fallback to `super.transaction(work)`. The raw service and raw store
are not returned.
...
The historical verifier MUST recompute this commitment from the exact stored
envelope, registered operation tuple, receiver-derived authentication, and
persisted host-context hash. A self-consistent, service-re-signed substitution
is rejected.
...
after the verification instant. The complete supplied chain is validated first;
a historical observation cannot bypass a malformed later suffix. It then
selects the observation's profile by the
...
accepted_failure:
  ...
  kernel:
    ...
    failures: [<closed frozen failure code>]
  wrapper_failure: null
...
The Phase-A controls exercise two distinct real committed non-success results
from `runtime_binding.get`: 422 `response_schema_mismatch` with its 16-access
callback history and 404 `object_not_found` with its 15-access history. Both
reserve exactly one nonce and commit a signed `accepted_failure` observation;
neither is rewritten into a wrapper error.
...
and crash durability remain Phase-B work.
...
| AS-11 | request signature proof/hash/key/controller/lifecycle or operation fingerprint is corrupted, or request/body/response is independently re-signed and substituted | exact stored envelope schema, object/body bindings, recomputed operation fingerprint, transaction-visible signing key, Ed25519 signature, request projection, query commitment, rich-origin binding, and result verifier reject |
...
| AS-24 | request context is missing after sealed bootstrap, nested over an active request, reused after installation, survives an intervening committed store generation, or is mismatched | a one-shot store-issued context lease is bound to its issuance global sequence and consumed on first installation attempt; missing/nested/reused/expired/mismatched admission rejects before callback/state change, an issued old token is directly rejected after another token commits, a nested attempt cannot replace the active context, and a fresh installed token completes the positive replay |
...
| AS-27 | duplicate, partial, forked, restarted, or concurrent genesis import | the complete candidate is verified in a detached sidecar before publication; a failed seal leaves sidecar and bootstrap state unchanged, while an exact duplicate returns the existing manifest hash without a second genesis row |
| AS-28 | a positional per-operation dependency/commit/scope/observation/repository/validation/receiver/host/request/trace/callback row is omitted, reordered, duplicated, added, history-altered, or has an extra/mismatched wrapper column; the order-independent operational-version tuple set is omitted, duplicated, backdated, negative, unwritten-positive, or future; the genesis manifest is rebound/extended; the closed service-commit sequence/time/shape changes; an unverified cache appears; or a stage counter differs | exact positional ledgers, sorted dependency entries, exact order-independent version tuples, pinned genesis inventory, closed service-commit row, exact counters, direct and independently repeated mutations, and recomputed historical roots reject |
...
| AS-35 | header/caller/raw namespace/tenant, principal, actor, runtime key, trust profile, authentication evidence, assertion level, account commitment, authority commitment, or opaque handle changes across two operations; the signed envelope principal or sender actor differs from the request-scoped receiver record; the namespace becomes operation-qualified; a rejected receiver record is cached or reused when the next request omits it; the receiver record differs from the frozen authentication/host projection; or the raw namespace appears in an observation/result | the mandatory transaction-owned receiver record, derived frozen authentication, closed host context, signed envelope principal/sender, and complete stable operation-independent binding by both account commitment and handle are enforced before callback and again before publication; real coherently signed principal/actor substitution requests traverse the wrapper and reject before the frozen callback with zero callback/state delta; operation-qualified and handle-only replays plus missing-record fallback reject; an explicit valid record can immediately complete a fresh replay; every public field has a direct mutation control; raw value never serializes into a caller artifact |
...
| AS-37 | every row of the closed outcome/commit matrix is fault-injected, including local-kernel versus HTTP-failure hashing | disposition, exact canonical kernel-result hash, observation presence, nonce, sequence, object, idempotency, and grant deltas match §7/§8.1 exactly; two real frozen `runtime_binding.get` callbacks commit distinct non-success shapes—422 `response_schema_mismatch` with 16 accesses and 404 `object_not_found` with 15—rather than using one synthetic or fixture-specific artifact |
| AS-38 | unknown field, nullable substitution, registry URI/tuple mutation, wrong union branch, `accepted_failure` claiming replay, replay claiming a non-success outcome, changed row column, stored operation-contract substitution, wrong self-hash/signature exclusion, query-commitment substitution, outcome swap, missing/noncommitted/byte-changed callback witness, callback/kernel/observation mismatch, or alternate JCS preimage is supplied to any entry point | strict schema/semantic/hash/signature/query/callback-witness/artifact verifier rejects; all nine deterministic entrypoint fixtures, success/replay and both real accepted-failure callback histories, real registry tuples, canonical vectors, and actual durable artifact bindings remain exact |
